...
popup.port.on('$addon:storage', function(storage) {
  console.log('$addon:storage', storage);
  console.log('isObject', isObject(storage));
...
popup.port.on('$addon:tabs', function(action) {
  if (action && action.method && action.args) {
    tabs[action.method].apply(tabs[action.method], action.args);
  }

  popup.port.emit('$addon:tabs', {
    activeTab: {
